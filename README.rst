...
rbh-find defines a ``-ls`` action that list current file like in (gnu-)find. The
output is in -ls standard output format. The block counts, which comes directly
from statx, are of 512bytes-blocks which is different from (gnu-)find.
