...
supports '-', '/' and '+' as a suffix for the mode string. The '+' is deprecated
and not used by GNU find but does not trigger a parsing error. Whereas, it is
a parsing error to use '+' in rbh-find as a suffix. Keep in mind that some
...
suffix and is the reason for the deprecation of '+' as a suffix.
