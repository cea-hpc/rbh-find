...
rbh-find defines the ``-sort`` and ``-rsort`` options which allow sorting entries
...
The "-sort / -rsort" has been implemented so that it affects everything
to its right, without taking into account any parentheses and or/and
expressions, allowing to combine the ability of rbh-find to add filters
as you go, with easy implementation.
