...
/**
 * duration2date - convert a fsentry mtime to a timestamp
 *
 * @param buf      a pointer to an empty buffer
 * @param mtime    a mtime in seconde
 *
...
duration2date(char *buf, uint64_t mtime)
{
    ...
    else if (datetime->tm_year < local.tm_year)
        strftime(buf, 25, "%b %e %G", datetime);
    else
        strftime(buf, 25, "%b %e %T", datetime);
      ...
      const struct rbh_filter *filter)
{
    ...
    do {
        ...
        case ACT_PRINT0:
            ...
            break;
        case ACT_LS:
            printf("%lld %lld %s %d %d %d %lld %s %s%c",fsentry->statx->stx_ino,
