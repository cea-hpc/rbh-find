...
test_xattr_not_exists()
{
    ...
    local output=$(rbh_find "rbh:mongo:$testdb" -xattr blob)
    if [ -n "$output" ]; then
        echo "rbh-find with xattr 'blob' should have returned nothing"
        return 1
    fi

    local output=$(rbh_find "rbh:mongo:$testdb" -xattr user.b)
    if [ -n "$output" ]; then
        echo "rbh-find with xattr 'user.b' should have returned nothing"
        return 1
    fi
}
