      ...
      size_t sorts_count)
{
    ...
    do {
        ...
        count += ctx->action_callback(ctx, action, fsentry);
        free(fsentry);
    } while (true);
                 ...
                 struct rbh_filter_sort **sorts, size_t *sorts_count)
{
    ...
    for (i = *arg_idx; i < ctx->argc; i++) {
        ...
        case CLT_PREDICATE:
            ...
            tmp = ctx->predicate_callback(ctx, &i);
            if (negate) {
                tmp = filter_not(tmp);
                negate = false;
            }
        ...
        case CLT_ACTION:
            ...
            ctx->find_callback(ctx, str2action(ctx->argv[i - 1]), &i,
                               &left_filter, *sorts, *sorts_count);
            break;
        }
    }
